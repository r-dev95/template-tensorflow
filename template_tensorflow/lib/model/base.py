...
class BaseModel(keras.models.Model):
    ...
    *   In graph mode, you can output too.
    ...
    def update_metrics(self, data: tuple[tf.Tensor]) -> None:
        """Updates metrics.

        Args:
            data (tuple[tf.Tensor]): tuple of labels, preds, loss, and sample_weight.
        """
        labels, preds, loss, sample_weight = data
        for m in self.classes[K.METRICS]:
            if not isinstance(m, keras.metrics.MeanMetricWrapper):
                m(loss, sample_weight=sample_weight)
            else:
    ...
    def train_step(self, data: tuple[tf.Tensor]) -> dict[str, float]:
        ...
        self.update_metrics(data=(labels, preds, loss, sample_weight))
